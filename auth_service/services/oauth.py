...
from core.oauth import oauth, OAuthProvider
from core.utils import get_random_password
from database.models import SocialAccount, SocialUser
from services import user as user_service, role as role_service
...
def get_user_from_social(social_name):
    ...
    if social_name == OAuthProvider.yandex.name:
        social_user = get_user_from_yandex(
            client.api_base_url,
            token['access_token'],
        )
    ...
    if not user:
        user = user_service.create_user_in_db(
            email=social_user.email,
            password=hash_password(get_random_password()),
        )
        users_role = role_service.get_role_by_name('user')
        user_service.add_role_to_user(user, users_role)

    create_social_account_in_db(
        user_id=user.id,
        social_id=social_user.id,
        social_name=social_name,
    )
...
def get_user_from_yandex(url, token):
    ...
    social_user = SocialUser(
        id=decoded_response['client_id'],
        email=decoded_response['default_email'],
    )
    return social_user
